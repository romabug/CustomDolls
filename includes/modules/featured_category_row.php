...
WHERE c1.categories_id=c.categories_id and parent_id = '.$featured_categories->fields['categories_id'] .' limit 5';
		...
		$subCatoryString .= '</dl><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '" class="more_product"> </a>';
		
		// see all, more product, more products.  下面修改首页类目图片大小
			//  可去掉 文字的 <h3 class="margin_t line_120 pad_b2"> </h3> CSS  
		
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<div><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '" title="'.SEO_COMMON_KEYWORDS.' '.$featured_categories->fields['categories_name'].'">' . zen_image(DIR_WS_IMAGES . $featured_categories->fields['categories_image'], $featured_categories->fields['categories_name'], 100, 100) . '</a></div> 
		 			   
			  <a  href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '" title="'.SEO_COMMON_KEYWORDS.' '.$featured_categories->fields['categories_name'].'">'. $featured_categories->fields['categories_name'] .'</a> '.$subCatoryString);
